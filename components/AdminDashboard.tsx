...
export function AdminDashboard({ users, logs, onBack, onRoleChange }: { users: Profile[]; logs: AuditLog[]; onBack: () => void; onRoleChange: (user: Profile, role: Exclude<UserRole, "admin">) => Promise<void> }) {
    ...
    <section className="fade-in mx-auto w-full max-w-7xl px-4 pb-28 pt-6 sm:px-6 lg:px-8">
      ...
      <div className="glass-dark relative overflow-hidden rounded-[2rem] p-6 text-white sm:p-9">
        ...
        <div className="relative flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
          ...
          <div className="flex gap-3"><Stat value={users.length} label="משתמשים" /><Stat value={editors} label="עורכים" /></div>
        </div>
      </div>

      <div className="mt-6 grid gap-6 xl:grid-cols-[1.05fr_.95fr]">
        <div className="glass overflow-hidden rounded-[1.75rem]">
          <div className="flex items-center justify-between border-b border-slate-200/70 p-5 sm:p-6"><div className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-violet-100 text-violet-600"><UsersRound size={20} /></span><div><h2 className="font-black">ניהול משתמשים</h2><p className="text-xs text-slate-500">הקצאת הרשאות צפייה ועריכה</p></div></div><span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-bold text-slate-500">{users.length} רשומים</span></div>
          <div className="divide-y divide-slate-100">
            {users.map((user) => <div key={user.id} className="flex flex-col gap-4 p-5 transition hover:bg-white/60 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-3"><span className="grid h-11 w-11 shrink-0 place-items-center rounded-full bg-gradient-to-br from-violet-500 to-cyan-500 font-black text-white">{user.name.charAt(0)}</span><div><p className="font-bold">{user.name}</p><p className="text-xs text-slate-500">{roleLabels[user.role]}</p></div></div>{user.role === "admin" ? <span className="flex items-center gap-1.5 self-start rounded-full bg-amber-50 px-3 py-1.5 text-xs font-bold text-amber-700 sm:self-auto"><ShieldCheck size={14} /> הרשאת מנהל</span> : <label className="relative"><span className="sr-only">בחירת הרשאה עבור {user.name}</span><select value={user.role} onChange={(event) => onRoleChange(user, event.target.value as "editor" | "viewer")} className="focus-ring min-w-32 appearance-none rounded-xl border border-slate-200 bg-white py-2.5 pe-9 ps-3 text-sm font-bold outline-none"><option value="viewer">צופה</option><option value="editor">עורך</option></select><UserCog className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={16} /></label>}</div>)}
          </div>
        </div>
...
function formatAction(action: string) {
  ...
  if (type === "role_updated") return { text: <>שינה את הרשאת <strong>{subject}</strong> ל־{role === "editor" ? "עורך" : "צופה"}</>, tone: "bg-amber-100 text-amber-700" };
