...
import { ArrowDownAZ, BookOpenText, BookPlus, ChevronLeft, Clock3, LogOut, Menu, Moon, Pencil, Presentation, Search, Sparkles, Sun, Trash2, UsersRound, X } from "lucide-react";
import type { Session } from "@supabase/supabase-js";
import { supabase, isSupabaseConfigured } from "@/lib/supabaseClient";
import { demoLogs, demoProfile, demoTerms, demoUsers } from "@/lib/demoData";
import { getLectureLabel, getLectureTone, lectures } from "@/lib/lectures";
import type { AuditLog, Profile, Term, TermFormValues, UserRole } from "@/types/database";
...
export function GlossaryApp() {
  ...
  const filteredTerms = useMemo(() => {
    ...
    const result = terms.filter((item) => {
      const matchesLecture = lectureFilter === "all" || item.lecture_id === lectureFilter;
      const matchesQuery = !normalized || item.term.toLocaleLowerCase("he").includes(normalized) || item.definition.toLocaleLowerCase("he").includes(normalized) || getLectureLabel(item.lecture_id).toLocaleLowerCase("he").includes(normalized);
      return matchesLecture && matchesQuery;
    });
  ...
  }, [lectureFilter, query, sort, terms]);
  ...
  return (
    <main className="mesh flex min-h-dvh flex-col">
      ...
      <div className="flex-1">{activeView === "admin" && profile.role === "admin" ? <AdminDashboard users={users} logs={logs} currentUserId={profile.id} onBack={() => setActiveView("glossary")} onRoleChange={changeRole} /> : <GlossaryView terms={filteredTerms} total={terms.length} query={query} setQuery={setQuery} lectureFilter={lectureFilter} setLectureFilter={setLectureFilter} sort={sort} setSort={setSort} canEdit={canEdit} setEditingTerm={setEditingTerm} setDeletingTerm={setDeletingTerm} />}</div>
      <footer className="border-t border-slate-200/70 bg-white/40 px-4 pb-[calc(1.25rem+env(safe-area-inset-bottom))] pt-5 text-center text-sm font-semibold leading-6 text-slate-500 backdrop-blur-xl">מילון המושגים פותח על ידי מתן סויסה (© 2026) במסגרת הקורס של תא&quot;ל (מיל&apos;) שגיב שרביט</footer>
...
function GlossaryView({ terms, total, query, setQuery, lectureFilter, setLectureFilter, sort, setSort, canEdit, setEditingTerm, setDeletingTerm }: { terms: Term[]; total: number; query: string; setQuery: (value: string) => void; lectureFilter: string; setLectureFilter: (value: string) => void; sort: SortMode; setSort: (value: SortMode) => void; canEdit: boolean; setEditingTerm: (term: Term | null) => void; setDeletingTerm: (term: Term) => void }) {
  return <section className="mx-auto w-full max-w-7xl px-4 pb-10 pt-8 sm:px-6 sm:pb-14 sm:pt-12 lg:px-8"><div className="slide-up grid gap-8 lg:grid-cols-[1fr_auto] lg:items-end"><div><div className="mb-4 inline-flex items-center gap-2 rounded-full border border-violet-200 bg-white/70 px-3 py-1.5 text-xs font-bold text-violet-600 shadow-sm"><span className="h-2 w-2 rounded-full bg-cyan-400" /> קורס 55-724 · תשפ״ו</div><h1 className="max-w-4xl text-4xl font-black tracking-[-0.035em] sm:text-6xl">עידן המידע בלוגיסטיקה ורכש: <span className="gradient-text">מאגר מושגים שיתופי</span> מתוך הרצאות מנהלים ויזמים בתעשייה 4.0.</h1><p className="mt-5 max-w-2xl text-base leading-8 text-slate-500 sm:text-lg">מילון חי ושיתופי מעולמות הדיגיטציה, האוטומציה והלוגיסטיקה החכמה - מבוסס על הרצאות הקורס ומקרי בוחן מהתעשייה.</p></div><div className="glass flex min-w-44 items-center gap-3 rounded-2xl p-4"><span className="grid h-11 w-11 place-items-center rounded-xl bg-cyan-100 text-cyan-700"><BookOpenText size={21} /></span><div><strong className="block text-2xl leading-none">{total}</strong><span className="text-xs text-slate-500">מושגים במילון</span></div></div></div>

    <div className="glass slide-up mt-10 flex flex-col gap-3 rounded-[1.6rem] p-3 lg:flex-row lg:items-center" style={{ animationDelay: "100ms" }}><label className="flex h-12 flex-1 items-center gap-3 rounded-2xl bg-white px-4 shadow-sm"><Search className="text-slate-400" size={20} /><input value={query} onChange={(event) => setQuery(event.target.value)} className="h-full w-full bg-transparent outline-none placeholder:text-slate-400" placeholder="חיפוש לפי מושג או תוכן ההגדרה..." aria-label="חיפוש במילון" />{query && <button onClick={() => setQuery("")} className="focus-ring rounded-lg p-1 text-slate-400" aria-label="ניקוי החיפוש"><X size={17} /></button>}</label><label className="relative flex h-12 min-w-0 items-center gap-2 rounded-2xl border border-slate-200/80 bg-white px-4 text-sm font-bold text-slate-600 lg:max-w-sm"><Presentation size={18} className="shrink-0 text-cyan-600" /><span className="sr-only">סינון לפי הרצאה</span><select value={lectureFilter} onChange={(event) => setLectureFilter(event.target.value)} className="min-w-0 flex-1 appearance-none truncate bg-transparent pe-5 outline-none"><option value="all">כל ההרצאות</option>{lectures.map((lecture) => <option key={lecture.id} value={lecture.id}>{lecture.label}</option>)}</select><ChevronLeft className="pointer-events-none absolute left-3 rotate-[-90deg] text-slate-400" size={15} /></label><label className="relative flex h-12 items-center gap-2 rounded-2xl border border-slate-200/80 bg-white px-4 text-sm font-bold text-slate-600"><ArrowDownAZ size={18} className="text-violet-600" /><span className="sr-only">מיון המושגים</span><select value={sort} onChange={(event) => setSort(event.target.value as SortMode)} className="appearance-none bg-transparent pe-5 outline-none"><option value="alphabetical">לפי א״ב</option><option value="newest">החדשים תחילה</option><option value="oldest">הוותיקים תחילה</option></select><ChevronLeft className="pointer-events-none absolute left-3 rotate-[-90deg] text-slate-400" size={15} /></label></div>

    <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">{terms.map((item, index) => <article key={item.id} className="glass term-card slide-up group flex min-h-64 flex-col rounded-[1.65rem] p-6" style={{ animationDelay: `${Math.min(index * 55, 330)}ms` }}><div className="flex items-start justify-between gap-3"><div className={`flex items-start gap-2 rounded-xl border px-3 py-2 text-xs font-semibold leading-5 ${getLectureTone(item.lecture_id)}`}><Presentation size={16} className="mt-0.5 shrink-0" /><span>{getLectureLabel(item.lecture_id)}</span></div>{canEdit && <div className="flex shrink-0 gap-1 opacity-100 transition sm:opacity-0 sm:group-hover:opacity-100 sm:group-focus-within:opacity-100"><button onClick={() => setEditingTerm(item)} className="focus-ring rounded-xl p-2 text-slate-400 transition hover:bg-violet-50 hover:text-violet-600" aria-label={`עריכת ${item.term}`}><Pencil size={17} /></button><button onClick={() => setDeletingTerm(item)} className="focus-ring rounded-xl p-2 text-slate-400 transition hover:bg-rose-50 hover:text-rose-600" aria-label={`מחיקת ${item.term}`}><Trash2 size={17} /></button></div>}</div><h2 className="mt-5 text-2xl font-black tracking-tight">{item.term}</h2><p className="mt-3 flex-1 text-sm leading-7 text-slate-600">{item.definition}</p><div className="mt-5 flex items-center justify-between border-t border-slate-200/70 pt-4 text-[11px] text-slate-400"><span className="flex items-center gap-1.5"><UsersRound size={14} /> {item.creator?.name ?? "חבר בקורס"}</span><span className="flex items-center gap-1.5"><Clock3 size={14} /> {new Intl.DateTimeFormat("he-IL", { day: "numeric", month: "short" }).format(new Date(item.created_at))}</span></div></article>)}</div>
    {terms.length === 0 && <div className="glass mt-6 rounded-[1.75rem] p-14 text-center"><span className="mx-auto grid h-14 w-14 place-items-center rounded-2xl bg-slate-100 text-slate-400"><Search size={24} /></span><h2 className="mt-5 text-xl font-black">לא נמצאו מושגים מתאימים</h2><p className="mt-2 text-sm text-slate-500">אפשר לשנות את החיפוש או לבחור הרצאה אחרת.</p></div>}
  </section>;
}
