...
export function GlossaryApp() {
  ...
  return (
    <main className="mesh flex min-h-screen flex-col">
      ...
      {canEdit && activeView === "glossary" && <button onClick={() => setEditingTerm(null)} className="focus-ring pulse-soft fixed bottom-7 left-1/2 z-30 flex h-16 -translate-x-1/2 items-center gap-2.5 whitespace-nowrap rounded-2xl bg-violet-600 px-7 text-base font-bold text-white shadow-2xl shadow-violet-500/30 transition hover:-translate-y-1 hover:bg-violet-700 sm:bottom-8" aria-label="הוספת מושג חדש"><BookPlus size={26} /><span>מושג חדש</span></button>}
      ...
      {notice && <div className="fade-in fixed bottom-24 left-1/2 z-[60] -translate-x-1/2 whitespace-nowrap rounded-2xl bg-slate-950 px-5 py-3 text-sm font-bold text-white shadow-2xl" role="status">{notice}</div>}
    </main>
  );
}
...
function LoadingScreen() { return <main className="grid min-h-screen place-items-center bg-slate-50"><div className="text-center"><span className="mx-auto grid h-14 w-14 animate-pulse place-items-center rounded-2xl bg-violet-600 text-white"><BookOpenText /></span><p className="mt-4 text-sm font-bold text-slate-500">טוענים את המילון...</p></div></main>; }
